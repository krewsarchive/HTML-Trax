...
function ler() {
	...
	if (no == 4)
		checkEnd();
	if ((!document.getElementById("moveRight").disabled && Number(agulha) + Number(origem) < 24) || document.getElementById("moveRight").disabled) {
		document.getElementById("agulha").value = Number(agulha) + Number(origem);
		console.log("AgulhaRange movida! " + (Number(agulha) + Number(origem)));
	} else if (!firstPlay) {
		document.getElementById("moveRight").click();
...
function save() {
	...
	if (!myCollections.length) { delete myCollections; }
	var layers = $(".layer");
	...
	$(layers).each(function (i) {
		...
		var spaces = $(this).find(".space");
		var index = 0;
		var ignoreNext = 0;

		$(spaces).each(function (ii) {
			...
			if (module.length > 0) {
				if (module.attr("trax-moduletype") == "unique" || module.attr("trax-moduletype") == "start") {
					var thisSong = module.attr('trax-playsong');
					mp3s.push({ track: songs[thisSong][0], len: songs[thisSong][2] });
					added = true;
					if (songs[thisSong][2] > 1) {
						ignoreNext = songs[thisSong][2] - 1;
					}
				}
			}

			if (!added) {
				if (mp3s.length == 0 || mp3s[mp3s.length - 1].track != 0) {
					mp3s.push({ track: 0, len: 1 });
				}
				else {
					mp3s[mp3s.length - 1].len += 1;
				}
			}
		...
		$(mp3s).each(function (index, value) {
			if (index == mp3s.length - 1 && value.track == 0)
				return;

			tempa.push(value.track + "," + value.len);
		});

		tracks.push((i + 1) + ":" + tempa.join(";"));

	});

	console.log(tracks.join(":"));

	console.log(`string: ${songString} length: ${songLength}`)
